...
class VisitorUpdateView(APIView):
    ...
    def get(self, request):
        try:
            ...
            visitor_profile = {"name": user.name, "email": user.email,
                               "phone": visitor.phone, "interests": visitor.interests, "city": visitor.city}
...
class LoginView(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            encoded_password = CustomUser.objects.filter(email=username)[
                0].password
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            if member_serializer.is_valid():
                ...
                # this should always work because depts are fixed
                department = Department.objects.filter(name=dept_name)[0]
...
class MemberProfileView(APIView):
    ...
    def get(self, request):
        try:
            user = request.user
            member = Member.objects.get(user=user)
            member_profile = {"name": user.name, "email": user.email, "bits_id": member.bits_id, "bits_email": member.bits_email, "department": str(
                member.department), "github": member.github, "codeforces_id": member.codeforces_id, "linked_in": member.linked_in, "skills": member.skills, "summary": member.summary}
